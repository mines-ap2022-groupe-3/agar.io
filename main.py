...
def main():
    ...
    while not done:
        ...
        position.x = u.clamp(position.x, 0, M_WIDTH)
        position.y = u.clamp(position.y, 0, M_HEIGHT)
        ...
        f.generate_fruit()
        blob_size = f.eat_fruit(position, size=blob_size)
        f.draw_fruits(screen, position)
        s.draw_blob(screen, size=blob_size, color=color)
