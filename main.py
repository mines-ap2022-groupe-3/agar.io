...
def main():
    ...
    while not done:
        ...
        new_direction = V2(pg.mouse.get_pos()) - V2(sc.SCREEN_CENTER)
        ...
        position.x = ut.clamp(position.x, 0, sc.M_WIDTH)
        position.y = ut.clamp(position.y, 0, sc.M_HEIGHT)
        ...
        fr.generate_fruit()
        blob_size = fr.eat_fruit(position, size=blob_size)
        fr.draw_fruits(screen, position)
